...
openShare = () => {
	Share.share(
		{
			message:
				"Hey Hommie, Check Out this Cole Quotes and Lyrics Game App, I promise you gon' love it, you can download it on AppStore and Google Play Store. ",
			url: '...',
			title: '',
		},
		{
...
const DashboardTabNavigator = createBottomTabNavigator(
	{
		Quotes: {
			...
			navigationOptions: {
				tabBarLabel: 'QUOTES',
				tabBarIcon: ({ tintColor }) => <Icon name="ios-quote" color={tintColor} size={24} />,
			},
		},
		'Lyrics Game': {
			screen: QuizScreen,
...
const DashboardStackNavigator = createStackNavigator(
	...
	},
	{
		defaultNavigationOptions: ({ navigation }) => {
			return {
				headerLeft: (
					<Icon style={{ padding: 10 }} onPress={() => navigation.openDrawer()} name="md-menu" size={30} />
				),
			};
		},
	}
...
const Menu = createStackNavigator({
	'Main Menu': {
		...
		navigationOptions: ({ navigation }) => ({
			...
			headerLeft: (
				<Icon onPress={() => navigation.openDrawer()} name="md-menu" style={{ padding: 10, fontSize: 30 }} />
			),
		}),
	},
});
		...
		Menu: {
			...
			navigationOptions: {
				...
				drawerIcon: ({ tintColor, focused }) => (
					<Ionicons name={focused ? 'md-menu' : 'md-menu'} size={26} style={{ color: tintColor }} />
				),
			},
		},
		Bio: {
