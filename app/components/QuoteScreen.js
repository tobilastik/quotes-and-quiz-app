...
export default class QuoteScreen extends Component {
	...
	nextQuote = () => {
		...
		if (activeQuoteIndex < quotes.length) {
			this.setState({
				activeQuoteIndex: Math.floor(Math.random() * (quotes.length - 1 - 0 + 1)) + 0,
			});
		}
	};
	...
	render() {
		...
		return (
			...
			<View style={styles.container}>
				<Text style={[styles.message,isFontLoaded && {fontFamily: 'Tulpen'}]}>{activeQuote.message}</Text>
				<Text style={[styles.author, isFontLoaded && {fontFamily: 'Prata'}]}>{activeQuote.author}</Text>
				<View style={{ flex: 1 }} />
				<TouchableOpacity block={true} onPress={this.nextQuote} style={styles.button}>
